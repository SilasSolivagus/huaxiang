...
| `js/records.js` | 记录页控制器：取数（只读解析）、4 个分类渲染、分类/二级列表切换、实时刷新 | `store.js`（personas 身份）、`board.js`（纯函数 `composeAgentSummary`） |
...
记录页**不 import 有副作用的 `Board` / `MemoryStream` 类**（它们会注册到全局 Map、调度存盘），改为只读直解析 localStorage，避免误写与触发存盘。仅复用 `store.js`（`loadConfig`/`runtimePersonas` 取人物身份，二者只读）与 `board.js` 的纯函数 `composeAgentSummary`。
